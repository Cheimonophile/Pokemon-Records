...
pub fn run() {
    ...
    let bianca = create_trainer(conn, "Bianca", &pkmn_trainer);
    let cheren = create_trainer(conn, "Cheren", &pkmn_trainer);
    ...
    let accumula_town = create_location(conn, "Accumula Town", "Unova");
    let n = create_trainer(conn, "N", &pkmn_trainer);
